        ...
        <div class="col-xl-10">
            <div class="card py-4 px-4">
                <div class="card-body">
                    ...
                    <form action="{{ route('descriptions.update', $update_id->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="col-12 mt-3">
                            <label for="title" class="form-label">Title <span class="text-danger">*</span></label>
                            <input name="title" id="title" cols="30" rows="5"
                                class="form-control" value="{{ $update_id->title }}"></input>
                            @error('title')
                        ...
                        <div class="col-12 mt-3">
                            ...
                            <textarea name="description" id="description" cols="30" rows="5" class="form-control editor">{{ $update_id->description }}</textarea>
                            @error('description')
                                <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
